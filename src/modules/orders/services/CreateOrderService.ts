...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) throw new AppError('Customer do not exists!');

    const existentProducts = await this.productsRepository.findAllById(
      products,
    );
    if (!existentProducts.length)
      throw new AppError('Could not find products with give ids');
    ...
    if (checkExistentProducts.length)
      throw new AppError(`Could not find product ${checkExistentProducts}`);
